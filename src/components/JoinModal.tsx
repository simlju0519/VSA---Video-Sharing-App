...
export default function JoinModal({ meeting, onClose, onNotify }: Props) {
  const handleNotifyAndJoin = () => {
    if (onNotify) onNotify(meeting.id);
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-end sm:items-center justify-center z-50" onClick={onClose}>
      <div className="bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-t-xl sm:rounded-xl p-5 sm:p-6 w-full sm:max-w-md sm:mx-4 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          {meeting.streamUrl && (
            <a href={meeting.streamUrl} target="_blank" rel="noopener noreferrer" className="flex-1 bg-red-600 text-white py-3 sm:py-2.5 rounded-lg hover:bg-red-500 transition font-medium text-center text-sm">
              📺 Open Stream ↗
            </a>
          )}
          <a href={meeting.joinUrl} target="_blank" rel="noopener noreferrer" onClick={handleNotifyAndJoin} className="flex-1 bg-green-600 text-white py-3 sm:py-2.5 rounded-lg hover:bg-green-500 transition font-medium text-center text-sm">
            {meeting.watchers?.length > 0 && !meeting.notified ? "🔔 Notify & Join ↗" : "Open Meeting ↗"}
          </a>
