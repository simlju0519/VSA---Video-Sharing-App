...
export default function BrowsePanel({ onWatchTogether, onSchedule }: Props) {
  const [step, setStep] = useState<"closed" | "categories" | "matches">("closed");
  ...
  const loadSHL = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/tv4?page=shl");
      const data = await res.json();
      const all: SportEvent[] = (data.panels || []).flatMap((p: { events: SportEvent[] }) => p.events);
  ...
  const handleSchedule = (e: SportEvent) => {
    onSchedule(e.title, streamUrl(e), e.startTime, e.endTime, e.homeTeamLogo || undefined, e.awayTeamLogo || undefined);
  ...
  if (step === "closed") {
    return (
      <button
        ...
        className="w-full py-4 rounded-lg border-2 border-dashed border-neutral-300 dark:border-neutral-600 text-neutral-500 dark:text-neutral-400 hover:border-blue-400 hover:text-blue-500 dark:hover:border-blue-500 dark:hover:text-blue-400 transition text-sm font-medium"
      >
  ...
  return (
    <div className="border border-neutral-200 dark:border-neutral-700 rounded-lg bg-white dark:bg-neutral-800/50 overflow-hidden">
      <div className="p-3 sm:p-4 border-b border-neutral-100 dark:border-neutral-700/50 flex justify-between items-center">
        ...
        <button onClick={() => { setStep("closed"); setMatches([]); setPendingEvent(null); }} className="text-xs text-neutral-400 hover:text-neutral-600 dark:hover:text-neutral-300 transition">✕ Close</button>
      </div>

      <div className="p-3 sm:p-4">
        {step === "matches" && !pendingEvent && (
          <button onClick={() => setStep("categories")} className="text-xs text-blue-500 hover:text-blue-400 mb-3 block">← Back</button>
        )}
        ...
        {pendingEvent && (
          <div className="rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/30 p-4">
            ...
            <div className="flex gap-2">
              <button
                onClick={() => handleSchedule(pendingEvent)}
                className="flex-1 py-2 rounded-md text-xs font-medium bg-blue-600 text-white hover:bg-blue-500 transition"
              >
                📅 Plan Watch Party
              </button>
              <button
                onClick={() => handleStartNow(pendingEvent)}
                className="flex-1 py-2 rounded-md text-xs font-medium bg-green-600 text-white hover:bg-green-500 transition"
              >
                ▶ Start Now Anyway
              </button>
              <button
                onClick={() => setPendingEvent(null)}
                className="px-3 py-2 rounded-md text-xs text-neutral-500 hover:text-neutral-700 dark:hover:text-neutral-300 transition"
              >
                Cancel
              </button>
            </div>
        ...
        {/* Categories */}
        {step === "categories" && !pendingEvent && (
          <button
            onClick={loadSHL}
            className="w-full flex items-center justify-between p-4 rounded-lg border border-neutral-100 dark:border-neutral-700/50 hover:border-blue-400 dark:hover:border-blue-500 hover:bg-blue-50/50 dark:hover:bg-blue-950/20 transition"
          >
            <div className="flex items-center gap-3">
              <span className="text-2xl">🏒</span>
              <div className="text-left">
                <p className="text-sm font-medium text-neutral-900 dark:text-white">SHL</p>
                <p className="text-xs text-neutral-400">Svenska Hockeyligan</p>
              </div>
            </div>
              ...
              {matches.map((e) => {
                ...
                return (
                  <button
                    ...
                    disabled={launchingId === e.id}
                    className="w-full flex items-center gap-3 p-3 rounded-lg border border-neutral-100 dark:border-neutral-700/50 hover:border-green-400 dark:hover:border-green-500 hover:bg-green-50/50 dark:hover:bg-green-950/20 transition text-left disabled:opacity-40"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-neutral-400 truncate">{e.readableTime} · {e.arena}{e.studio ? " · 📺 Studio" : ""}</p>
                    </div>
